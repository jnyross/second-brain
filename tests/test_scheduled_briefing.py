...
class TestInstallScript:
    ...
    def test_install_script_checks_root(self, install_script: Path):
        ...
        content = install_script.read_text()
        assert "EUID" in content or "root" in content, "Script should check for root privileges"
...
class TestBriefingCLI:
    ...
    async def test_send_briefing_generates_content(self):
        ...
        with (
            patch("assistant.cli.settings") as mock_settings,
        ...
        mock_bot.send_briefing.assert_called_once_with(
            "123456789", "Good morning! Here's your day..."
        )
...
class TestAT106:
    ...
    def test_timer_scheduled_for_7am(self):
        ...
        assert "07:00:00" in content or "07:00" in content, "Timer should be scheduled for 7:00 AM"
    ...
    def test_timer_will_catch_up_if_missed(self):
        ...
        assert "Persistent=true" in content, "Timer should be persistent to catch up on missed runs"
